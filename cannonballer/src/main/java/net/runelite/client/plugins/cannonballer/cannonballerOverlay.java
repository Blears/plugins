...
@Slf4j
@Singleton
class cannonballerOverlay extends OverlayPanel
{
	private final cannonballerPlugin plugin;
	private final cannonballerConfiguration config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private cannonballerOverlay(final Client client, final cannonballerPlugin plugin, final cannonballerConfiguration config)
	{
		super(plugin);
		setPosition(OverlayPosition.BOTTOM_LEFT);
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Cannonballer overlay"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startCannonBaller || !config.enableUI())
		{
