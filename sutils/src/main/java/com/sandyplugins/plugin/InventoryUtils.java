...
public class InventoryUtils {
    ...
    public void dropItem(WidgetItem item) {
        ...
        menu.setEntry(new LegacyMenuEntry("", "", item.getId(), MenuAction.ITEM_FIFTH_OPTION, item.getIndex(), 9764864, false));
    ...
    public void itemsInteract(Collection<Integer> ids, MenuAction type, boolean exceptItems, boolean interactAll, int minDelayBetween, int maxDelayBetween) {
        Collection<WidgetItem> inventoryItems = getAllItems();
        executorService.submit(() ->
        {
            try {
                ...
                for (WidgetItem item : inventoryItems) {
                    if ((!exceptItems && ids.contains(item.getId()) || (exceptItems && !ids.contains(item.getId())))) {
                        ...
                        sleep(minDelayBetween, maxDelayBetween);
                        menu.setEntry(new LegacyMenuEntry("", "", item.getId(), type, item.getIndex(), WidgetInfo.INVENTORY.getId(),
                                true));
                        mouse.click(item.getCanvasBounds());
                        if (!interactAll) {
                            break;
    ...
    public void combineItems(Collection<Integer> ids, int item1ID, MenuAction type, boolean exceptItems, boolean interactAll, int minDelayBetween, int maxDelayBetween) {
            ...
            try {
                ...
                for (WidgetItem item : inventoryItems) {
                    if ((!exceptItems && ids.contains(item.getId()) || (exceptItems && !ids.contains(item.getId())))) {
                        log.info("interacting inventory item: {}", item.getId());
                        sleep(minDelayBetween, maxDelayBetween);
                        menu.setModifiedEntry(new LegacyMenuEntry("", "", item1.getId(), type, item1.getIndex(), WidgetInfo.INVENTORY.getId(),
